...
func HandleNetworkStatus(ctx Context, params interface{}) error {
	...
	err := helper.Map2Struct(params, &networkStatus)
	if err != nil {
		return err
	}
	...
	if networkStatus.Networked {
		...
		if err := ctx.CollectAndReportProducts(); err != nil {
			helper.Logger.Error("Failed to report products", zap.Error(err))
			return err
		}
		helper.Logger.Info("Networked, reporting device, model and driver lists")
	}
	return nil
}
